...
from medmnist.inception import InceptionV3
from medmnist.resnet import ResNet18, ResNet50
from medmnist.densenet import DenseNet121, DenseNet161, DenseNet169, DenseNet201
...
def main(flag, input_root, output_root, end_epoch, download, model_name):
    ''' main function
    ...
    print(f'Utilizing device = {device}')  # reporting for debug information
    # NOTE: decide on model based on argument
    if(model_name == 'ResNet18'):
        ...
        model = DenseNet121(in_channels=n_channels, num_classes=n_classes).to(device)
    elif(model_name == 'DenseNet161'):
        model = DenseNet161(in_channels=n_channels, num_classes=n_classes).to(device)
    elif(model_name == 'DenseNet169'):
        model = DenseNet169(in_channels=n_channels, num_classes=n_classes).to(device)
    elif(model_name == 'DenseNet201'):
        model = DenseNet201(in_channels=n_channels, num_classes=n_classes).to(device)
    elif(model_name == 'InceptionV3'):
        model = InceptionV3(in_channels=n_channels, num_classes=n_classes).to(device)
    else:
